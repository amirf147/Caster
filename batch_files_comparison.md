...
This document provides a terse comparison of the speech-recognition batch scripts available in the Caster directory.

---

## 1. Run_Caster_Kaldi.bat
* **Path**: `Run_Caster_Kaldi.bat`
...
* **Path**: `Run_Caster_Kaldi_Latest.bat`
...
## 4. Run_Caster_Kaldi_Latest_retain_audio_and_commands.bat
* **Path**: `Run_Caster_Kaldi_Latest_retain_audio_and_commands.bat`
* **Python Executable**: Local Virtual Environment Python (`.venv_latest\Scripts\python.exe`)
* **Virtual Environment**: **Yes** (`.venv_latest` directory inside Caster folder)
* **Dragonfly & Pyvda Source**: Local repos at `repos\pyvda` and `repos\dragonfly`
* **Data Retention**: **Enabled** (`retain_dir=saved_audio` creates `saved_audio/` and stores `retain.tsv` logs + `.wav` audio files)
* **Command Executed**:
  ```bat
  set PYTHONPATH=%USERPROFILE%\Documents\repos\pyvda;%USERPROFILE%\Documents\repos\dragonfly
